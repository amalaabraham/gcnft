        ...
        const encodedMetadata = await fcl.send([
          fcl.script`
            ...
            pub fun main(id: Int) : {String : String} {
              ...
              let capability = nftOwner.getCapability<&{commitContract.NFTReceiver}>(/public/NFTReceiver)
 ...
 /* const buyToken = async (tokenId) => {
    ...
    .send([
      ...
      fcl.transaction`
        ...
        import MarketplaceContract from 0xf8d6e0586b0a20c7
        transaction {
            let collectionRef: &AnyResource{commitContract.NFTReceiver}
            let temporaryVault: @CommitToken.Vault
            prepare(acct: AuthAccount) {
                self.collectionRef = acct.borrow<&AnyResource{commitContract.NFTReceiver}>(from: /storage/NFTCollection)!
